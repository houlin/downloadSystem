...
import com.jams.music.player.Bean.NewLocal;
...
public class DownloadManager extends Thread {
    ...
    private void broadcastAddTask(String url, boolean isInterrupt) {
        ...
        Intent nofityIntent = new Intent(DownloadConst.TAG_NAME_NOTIFY_MAINACTIVITY);
    ...
    public void checkUncompleteTasks() {
        List<NewLocal> urlList = mApp.getDBAccessHelper().getLoadNewLocalUndo();
        if (urlList.size() >= 0) {
            for (int i = 0; i < urlList.size(); i++) {
                addTask(urlList.get(i).getRemotePath());
            }
        }
    }
        ...
        DownloadTaskListener taskListener = new DownloadTaskListener() {
            ...
            public void updateProcess(DownloadTask task) {
                ...
                Intent updateIntent = new Intent(
                        DownloadConst.TAG_NAME_NOTIFY_MAINACTIVITY);
